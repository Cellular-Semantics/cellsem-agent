...
from .cell_config import CellDependencies, get_config

logger = logging.getLogger(__name__)


async def get_info(query: str, history: List[str], deps: CellDependencies, **kwargs) -> str:
    """
    ...
    result = await cell_agent.run(query, deps=deps, **kwargs)
    return result.data


def chat(deps: Optional[CellDependencies] = None, model=None, **kwargs):
    """
    Create a Gradio chat interface for the Cell agent.
